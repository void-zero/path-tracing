...
void Buffer::save( const std::string &filename ) const
{
    ...
    for ( unsigned int y = 0; y < v_resolution_; y++ )
    {
        for ( unsigned int x = 0; x < h_resolution_; x++ )
        {
            ...
            rendering_file << static_cast< int >( powf( clamp(buffer_data_[x][y][0]), GAMMA ) * 255.0f + 0.5f ) << " ";
            rendering_file << static_cast< int >( powf( clamp(buffer_data_[x][y][1]), GAMMA ) * 255.0f + 0.5f ) << " ";
            rendering_file << static_cast< int >( powf( clamp(buffer_data_[x][y][2]), GAMMA ) * 255.0f + 0.5f ) << " ";
        }
    }
